---
name: church-bible-study
description: "Use this skill for Bible study group preparation, small group curriculum design, individual book studies, topical studies, and character studies. Generates: multi-week series outlines, session-by-session study guides, deep passage analysis, discussion question banks, cross-references, and historical-cultural context. Triggers: 'Bible study', 'small group', 'study series', 'book study', 'exegesis', 'passage analysis', 'discussion questions', 'inductive Bible study'."
license: MIT
pack: church
---

# 📖 The Scholar — Bible Study Skill

This skill supports Bible study facilitators in guiding groups into deep, transformative encounter with Scripture. It balances academic rigour with pastoral accessibility — helping ordinary believers engage the Word at the highest level without needing a seminary degree.

---

## Three Study Modes

### Mode 1: Book Study (Verse-by-verse / Section-by-section)
For groups wanting to journey through a complete biblical book.

**Multi-Week Series Output:**
```
SERIES: [Book Name] — [Series Title]
DURATION: [X weeks]
TARGET AUDIENCE: [New believers | Growing disciples | Mature saints | Mixed]
STUDY APPROACH: [Expository | Inductive | Narrative | Theological]

OVERVIEW:
  Author: [Name, background, relationship to audience]
  Date: [Approximate period]
  Recipients: [Who received this letter/book originally]
  Purpose: [Why was this written? What situation prompted it?]
  Structure: [How the book is organised — key sections]
  Theological Heart: [The core message/claim of the book]
  Relevance Today: [Why this book matters for 21st century readers]

SESSION MAP:
  Session 1: [Title] | [Passage] | [Theme] | [Key Question]
  Session 2: [Title] | [Passage] | [Theme] | [Key Question]
  [...]
  Final Session: [How the series culminates and what response is invited]
```

### Mode 2: Topical Study
For groups studying a theological theme across multiple biblical texts.

Topics handled: Prayer · Forgiveness · Suffering · Hope · Identity · Justice · Grace · Money · Relationships · Fear · Purpose · Spiritual Disciplines · The Holy Spirit · Eschatology · Mission

**Output includes:**
- 4–8 session structure following the biblical narrative arc of the theme
- Key texts per session (not proof-texting — full passage context)
- How the theme develops from OT → NT
- Practical application per session
- Recommended resources for deeper study

### Mode 3: Character Study
For groups studying a biblical figure's faith journey.

Characters covered: Abraham · Moses · Esther · Ruth · David · Elijah · Mary · Peter · Paul · Mary Magdalene · Joseph · Daniel · Nehemiah + more

**Output structure:**
- Character introduction and historical setting
- Session-by-session narrative moments
- Character flaws and failures (not hagiography)
- Moments of faith and transformation
- How their story points to Christ
- Application: "What does [character]'s journey reveal about God and about us?"

---

## Session Study Guide Template

```
═══════════════════════════════════════════════
📖 BIBLE STUDY SESSION [Number]
═══════════════════════════════════════════════
SESSION TITLE: [Engaging title]
PASSAGE: [Reference]
SERIES: [If applicable]

FACILITATOR PREPARATION NOTES:
  [Key interpretive points the leader must understand before leading]
  [Potential difficult questions the group may raise — with guidance]
  [Recommended reading: commentaries, articles]

───────────────────────────────────────────────
HISTORICAL-CULTURAL CONTEXT (5–10 min share)
  [Background the group needs to understand before reading]
  [Keep to 3–5 key facts — not a lecture]

───────────────────────────────────────────────
READ THE PASSAGE TOGETHER
  [Reading instructions: reader rotation, dramatic reading, silent reading]
  [Note-taking prompt for group members]

───────────────────────────────────────────────
OBSERVATION QUESTIONS (What does the text SAY?)
  1. [Specific, text-based question]
  2. [Structural or narrative observation]
  3. [Character/speaker observation]

INTERPRETATION QUESTIONS (What does the text MEAN?)
  4. [Why does the author say this here?]
  5. [What is the theological claim?]
  6. [What is surprising, confusing, or challenging?]
  7. [How does this connect to the larger biblical story?]

APPLICATION QUESTIONS (What does this mean for US?)
  8. [Personal reflection — what does this reveal about God?]
  9. [Community application — what does this mean for how we live together?]
  10. [This week action — one specific response to this passage]

───────────────────────────────────────────────
CLOSING PRAYER GUIDE
  [Specific intercession points drawn from the passage]
  [Invitation to respond in prayer as a group]

BETWEEN SESSIONS
  [Scripture to meditate on before next meeting]
  [Optional deeper study resource]
  [Suggested personal practice to apply the session's truth]
═══════════════════════════════════════════════
```

---

## Passage Deep-Dive Analysis

When a facilitator needs detailed exegetical notes on a passage:

### Structural Analysis
- Paragraph/strophe breaks
- Narrative arc or argument flow
- Key transitions and connectives
- Climactic verse identification

### Word Studies
For 2–4 key terms:
```
WORD: [English term]
ORIGINAL: [Greek/Hebrew]
MEANING RANGE: [Semantic range]
USAGE IN THIS CONTEXT: [What it means here specifically]
OTHER OCCURRENCES: [How the same word is used elsewhere in Scripture]
SIGNIFICANCE: [Why this word choice matters for interpretation]
```

### Cross-Reference Map
```
PASSAGE: [Reference]
DIRECT QUOTES: [OT passages directly quoted]
ALLUSIONS: [Subtler references and echoes]
THEMATIC PARALLELS: [Other passages on the same theme]
CONTRAST TEXTS: [Passages that nuance or qualify]
CHRISTOLOGICAL CONNECTION: [How this passage points to or is fulfilled in Christ]
```

### Difficult Texts Protocol
For passages that raise theological, ethical, or cultural questions:
1. **Acknowledge the difficulty** — do not smooth it over
2. **Present the major interpretive positions** (2–3 main views)
3. **Note which traditions tend toward which interpretation**
4. **Offer a framework for discussion, not a verdict**
5. **Pastoral note for facilitators**: how to hold the tension with grace

Examples: divine violence (Joshua), imprecatory Psalms, women and church leadership, Paul on slavery, apocalyptic imagery

---

## Small Group Dynamics Guidance

For facilitators managing group health:

### Starting a New Group
- Session 1 covenant guide: confidentiality, respect, participation norms
- Introductory icebreakers that build trust
- How to establish a safe space for honest questions

### Managing Difficult Group Members
- The talker who dominates
- The silent member who disengages
- The challenger who debates everything
- The hurting member who turns every session personal
- The expert who corrects the facilitator

(Guidance is pastoral and practical — not clinical management frameworks)

### Group Milestones
- Celebrate biblical literacy growth
- End-of-series reflection activity
- Testimony sharing structure
- What to do when the group is ready to multiply

---

## Recommended Commentary Library

When a facilitator asks for deeper resources, provide:
- **Accessible** (for general group members): The Bible Speaks Today series, IVP Bible Guides, Life Application Commentary
- **Intermediate** (for facilitators): NIV Application Commentary, Pillar NT Commentary
- **Academic** (for pastor-level preparation): Word Biblical Commentary, New International Greek Testament Commentary
- **Free Digital Resources**: Bible Gateway, Blue Letter Bible, BibleProject videos (thematic)
