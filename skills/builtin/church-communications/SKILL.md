---
name: church-communications
description: "Use this skill for ALL church external and internal communications: social media posts, newsletter articles, welcome emails, event invitations, prayer circulars, announcement scripts, testimony features, community outreach copy, and digital content. Supports multi-language church communities. Triggers: 'announcement', 'social media', 'newsletter article', 'email campaign', 'welcome email', 'invitation', 'press release', 'testimony', 'church post'."
license: MIT
pack: church
---

# 📣 The Herald — Church Communications Skill

The church's communication is its public face and its community heartbeat. This skill crafts communications that are warm, inclusive, theologically honest, and genuinely engaging — so the right people hear the right message at the right moment.

_"Let your conversation be always full of grace, seasoned with salt, so that you may know how to answer everyone."_ — Colossians 4:6

---

## Communication Principles

1. **Grace-first language** — assume the reader is a spiritual seeker, not a theological expert
2. **Action-oriented** — every communication has one clear next step
3. **Authentic, not corporate** — church communications should sound like people, not institutions
4. **Inclusive** — welcomes the first-time visitor as much as the 30-year member
5. **Timely** — gives people enough notice to respond and plan

---

## Social Media Content

### Platform-Specific Templates

**Instagram / Facebook Post — Event Announcement**

```
[Attention-grabbing first line — question or bold statement]

[1–2 sentences about the event — what, why it matters]

📅 [Date & Time]
📍 [Location]
🔗 [Link or "message us to register"]

[Relevant hashtags — church-appropriate, location-based, thematic]
```

**Instagram Story — Quote Graphic Script**

```
[Short Scripture verse or powerful quote — max 12 words for readability]
Typeface: Bold, high contrast
Background: [Suggested colour palette — seasonal / theme-aligned]
CTA: "Save this ↑" or "Share with someone who needs this"
```

**Twitter/X — Sermon Quote Drop**

```
"[Memorable sermon quote — under 240 chars, no filler]"

— [Preacher Name], [Church Name]
[Service date context]

[1–2 relevant hashtags]
```

**YouTube / Podcast Description**

```
[Episode/Sermon Title] | [Church Name]

In this [message/episode], [Pastor Name] unpacks [passage/theme],
exploring [what the listener will discover].

Perfect for anyone who [relatable situation — grief, doubt, seeking purpose, etc.]

📖 Key Scripture: [Reference]
🕐 [Timestamp for key moments if applicable]

Subscribe for weekly messages of hope and truth.
[Social links]
```

---

## Email Campaigns

### New Member Welcome Sequence (3-email series)

**Email 1 — Day 1: Warm Welcome**

```
Subject: You're home 🏠

Dear [First Name],

We noticed your name for the first time on Sunday, and we couldn't be more grateful you came.

[Church Name] isn't a building — it's a community of people who are figuring out faith,
life, and love together. You belong here, whether you've been a Christian for 30 years
or you're still not sure what you believe.

Here's what's available to you right now:
  → Our small groups (great way to connect): [link]
  → This week's sermon if you missed it: [link]
  → Meet [Pastor Name]: [booking link or phone]

We don't want to overwhelm you — just wanted you to know we see you and we're glad you're here.

With warmth,
[Church Team Name]
```

**Email 2 — Day 7: Connection Invitation**

```
Subject: One simple way to go deeper

Hi [First Name],

It's been a week since you first visited, and we've been thinking about you!

The best thing we've found for people who want to go deeper is [small group / Alpha course /
newcomers' event]. It's [brief description — low commitment, no pressure].

Next one is [Date] at [Time] at [Location].

Would you like to come? Just reply to this email — no forms, no fuss.

See you soon,
[Name]
```

**Email 3 — Day 30: Month-in Check-in**

```
Subject: A month in — how are you finding it?

Hi [First Name],

A month ago you walked through our doors for the first time — or maybe it was the first time
in a long time. Either way, we wanted to check in.

How are you finding things? Is there anything we can do to help you feel more at home?

[If not yet connected:] There's still space in our [small group / newcomers' gathering] —
we'd love to introduce you to a few people.

[If already connected:] It's been brilliant having you around.

No pressure — just wanted you to know we're still glad you're here.

[Pastor's name], [Church Name]
```

---

## Announcement Scripts (For Sunday Service)

### Standard Announcement Script Template

```
ANNOUNCEMENT SCRIPT: [Event/Initiative]
─────────────────────────────────────────
HOOK: [One sentence that makes people want to listen]

WHAT: [Clear description in 2 sentences]

WHY IT MATTERS: [1 sentence — the heart behind it]

WHO IT'S FOR: [Specific invite — "this is for you if..."]

HOW TO GET INVOLVED: [One simple action step]

WHEN/WHERE: [Date, time, location]

CLOSING LINE: [Brief, warm, not salesy]

[Optional: Personal testimony prompt — "If you've been to [event] before, tell someone
what it meant to you this week."]
```

---

## Testimony Features

### Written Testimony Guide

Help congregation members share their faith story effectively:

**Structure:**

1. **Before** — life before faith (or during a difficult season) — honest, relatable
2. **The moment** — what happened, what changed, when you encountered God
3. **After** — what's different now — specific, not overly triumphalistic
4. **Invitation** — what you want others to know they can find

**Length:** 300–500 words for written; 2–3 minutes spoken

**Testimony Coaching Notes for the member:**

- You don't need a dramatic story — ordinary faithfulness is powerful
- Don't feel pressure to have "all the answers" now
- Speak about your experience, not others'
- It's okay to include ongoing struggles — authenticity resonates more than perfection

---

## Community Outreach Copy

### Neighbourhood Event Invitation

```
[Event Name]
[Date] | [Time] | [Location]

You're invited — no catch, no commitment required.

[One sentence about what happens at the event]

This is for [target audience — "everyone in the neighbourhood", "local families", etc.]

Free entry | [Other relevant details]

Organised by [Church Name] | [Contact] | [Website]

"We believe every person matters. Come and see."
```

### Alpha / Christianity Explored Invitation

```
Subject: Questions about life? This is for you.

What's the point of it all?
Is there more to life than this?
What happens when we die?

These are big questions. At [Church Name], we think they deserve honest,
pressure-free conversation.

[Alpha / Christianity Explored] is a [X]-week series where you can explore the
Christian faith with complete freedom to doubt, disagree, and ask anything.

Starting: [Date]
Day/Time: [Details]
Venue: [Location + online option if applicable]

No experience required. No commitment implied. Just honest conversation,
good food, and real community.

[Registration link / contact]
```

---

⚠️ **All communications must be reviewed by the appropriate church leader before sending. This skill produces draft content only. The church maintains editorial responsibility for all published material.**
