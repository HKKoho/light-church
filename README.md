...
- Tools come in three kinds: **built-in** (a Light Church page, e.g. Game Builder; Mission/Camp Companion — where ministry leaders plan a trip or camp's schedule, devotionals, songs, photos and notes for the whole team; and Roll Call — a Simple mode (the original 茶果嶺浸信會點名應用程式) and a Smart mode with attendance history, pastoral-care reminders for who hasn't been in, pandas-style analysis by sex and age, and automatic duplicate-name merging; member data never leaves the server), **hosted** (a ready-made app that runs inside Light Church in a locked-down sandbox, e.g. Sunday Service Bulletin) and **external links** (open in a new tab, with a reminder not to paste members' personal data, e.g. Finance Pipeline).
