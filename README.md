...
- **Default tools** live in the repo under `ai-tools/` (`finance-pipeline`, `roll-call`, `mission-camp-companion`, `sunday-service-bulletin`). A `tool.json` may use `${VAR:-default}` placeholders, filled from the environment or `.env` at install time — e.g. the Finance Pipeline link reads `FINANCE_PIPELINE_URL` (default `http://localhost:3000`). Each folder name is the tool's id; `tool.json` sets `displayName` and `description` per language (`{ "en": "…", "zh-TW": "…" }`). Install them with `node scripts/seed-ai-tools.mjs`; existing tools are kept unless you pass `--force`.
